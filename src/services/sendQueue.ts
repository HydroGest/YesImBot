...
export class SendQueue {
  ...
  async checkQueueSize(channelId: string): Promise<boolean> {
    return (await this.queueManager.getQueue(channelId)).length > this.slotSize;
  }
  ...
  async checkMixedQueueSize(channelId: string): Promise<boolean> {
    for (let slotContain of this.slotContains) {
      if (slotContain.has(channelId)) {
        return (await this.queueManager.getMixedQueue(slotContain)).length > this.slotSize;
      }
    }
    return false;
  }
  ...
  async getQueue(channelId: string): Promise<ChatMessage[]> {
    return this.queueManager.getQueue(channelId);
  }

  async getMixedQueue(channelId: string): Promise<ChatMessage[]>{
    for (let slotContain of this.slotContains) {
        if (slotContain.has(channelId)) {
          return await this.queueManager.getMixedQueue(slotContain);
        }
      }
      return [];
  }
