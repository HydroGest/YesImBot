...
describe("Conversation.compact", () => {
  ...
  it("uses the supplied immutable LLM/persona snapshot and only switches after a valid summary", async () => {
    ...
    await conversation.storage.append(createEntry("message", { id: "m1", timestamp: 1, role: "user", content: "first" }), createEntry("message", { id: "m2", timestamp: 2, role: "assistant", content: "second" }));
    ...
    expect(generateText).toHaveBeenCalledWith(expect.objectContaining({ model: expect.anything(), system: expect.stringContaining("Athena"), prompt: expect.stringContaining("persona") }));
  ...
  it("does not activate a new session for an empty model summary", async () => {
    ...
    await expect(conversation.compact("manual", { model: {} as never, personaName: "Athena", persona: "persona" })).resolves.toMatchObject({ compacted: false, reason: "empty_summary" });
  ...
  it("skips model compaction below the minimum message count", async () => {
    ...
    await conversation.init();
    await conversation.storage.append(createEntry("message", { id: "m1", timestamp: 1, role: "user", content: "first" }), createEntry("message", { id: "m2", timestamp: 2, role: "assistant", content: "second" }));
    await expect(conversation.compact("auto", { model: {} as never, personaName: "Athena", persona: "persona" })).resolves.toEqual({ compacted: false, reason: "minimum_messages" });
  ...
  it("keeps the active session when the model fails", async () => {
    ...
    await expect(conversation.compact("manual", { model: {} as never, personaName: "Athena", persona: "persona" })).resolves.toEqual({ compacted: false, reason: "model_failure" });
  ...
  it("stops trying after the configured consecutive failure limit", async () => {
    ...
    await conversation.storage.append(createEntry("message", { id: "m1", timestamp: 1, role: "user", content: "first" }), createEntry("message", { id: "m2", timestamp: 2, role: "assistant", content: "second" }));
    await conversation.compact("manual", { model: {} as never, personaName: "Athena", persona: "persona" });
    await expect(conversation.compact("manual", { model: {} as never, personaName: "Athena", persona: "persona" })).resolves.toEqual({ compacted: false, reason: "failure_limit" });
