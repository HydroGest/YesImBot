...
vi.mock("@yesimbot/agent-runtime", async (original) => {
  ...
  return { ...actual, createAgent: vi.fn(() => ({ init: vi.fn(), append: vi.fn().mockResolvedValue(undefined), send: vi.fn(), run: vi.fn(() => (async function* () {})()), getActiveTurnId: () => state.active, wait: vi.fn(), interrupt: vi.fn(), stop: vi.fn(), isIdle: () => true })) };
});
...
async function createRuntime() {
  ...
  const value = new ChannelRuntime(new Context(), { channel, bot: { selfId: "bot", sendMessage: vi.fn() } as never, will: { decide: vi.fn().mockResolvedValue("wait"), observe: vi.fn() } as never, model: {} as never, imageOutputSupported: false, idleTimeout: config.session.idle.timeout, config, plugins: [] });
  await value.init();
  return { value, channel, root };
}

describe("ChannelRuntime idle compaction", () => {
  beforeEach(() => { vi.useFakeTimers(); state.active = null; });
