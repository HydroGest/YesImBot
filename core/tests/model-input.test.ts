...
import { createEvent, createMessage, type Event, type EventRecord, type Input, type MessageRecord } from "../src/input.js";
...
function plugin(
  ...
  imageBudget: { readonly maxCount: number; readonly maxBytesPerImage: number; readonly maxTotalBytes: number } | null = {
    maxCount: 4,
...
describe("createModelInputPlugin", () => {
  ...
  it("does not read images when image input is disabled", async () => {
    ...
    await expect(project(input, context([input]), plugin(assets, null))).resolves.toMatchObject({ role: "user" });
  ...
  it("keeps text and records diagnostics for invalid or missing assets", async () => {
    ...
    const input = createMessage(messageRecordWithText('<img id="55555555555555555555555555555555"/><img id="66666666666666666666666666666666"/><img id="77777777777777777777777777777777"/>'));
  ...
  it("charges each reference and skips oversized candidates while accepting later files", async () => {
    ...
    const input = createMessage(messageRecordWithText('<img id="88888888888888888888888888888888"/><img id="99999999999999999999999999999999"/><img id="99999999999999999999999999999999"/><img id="aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"/>'));
    const result = await project(input, context([input]), plugin(assets, { maxCount: 2, maxBytesPerImage: 8, maxTotalBytes: 16 }));
  ...
  it("accepts exact per-image and total limits and resets the budget for each context", async () => {
    ...
    const first = createMessage(messageRecordWithText('<img id="bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"/><img id="cccccccccccccccccccccccccccccccc"/><img id="dddddddddddddddddddddddddddddddd"/>'));
