...
export class ChannelStorage {
  ...
  private async assertChannelRoot(channelRoot: string): Promise<void> {
    if ((await fs.lstat(channelRoot)).isSymbolicLink()) throw new Error("Channel directory is a symbolic link");
  ...
  private async ensureChannel(scope: ChannelScope): Promise<ChannelManifest> {
    ...
    try {
      ...
      if (channelStorageKey(scopeFromManifest(manifest)) !== key) throw new Error("Channel storage integrity mismatch");
...
function encodeDirectoryComponent(value: string): string {
  ...
  for (const codePoint of value) {
    encoded += /[A-Za-z0-9]/.test(codePoint) ? codePoint : `%${codePoint.codePointAt(0)?.toString(16)}%`;
  }
  return encoded;
}
...
export function channelDirectoryName(scope: ChannelScope): string {
  ...
  const directoryName = (
    scope.type === "direct"
      ...
      : ["shared", encodeDirectoryComponent(scope.platform), encodeDirectoryComponent(scope.channelId)]
  ).join("-");
...
function parseManifest(value: unknown): ChannelManifest {
  ...
  if (typeof manifest.platform !== "string") throw new Error("Channel manifest platform is invalid");
  if (typeof manifest.channelId !== "string") throw new Error("Channel manifest channelId is invalid");
  if (typeof manifest.selfId !== "string") throw new Error("Channel manifest selfId is invalid");
  if (typeof manifest.createdAt !== "string") throw new Error("Channel manifest createdAt is invalid");
