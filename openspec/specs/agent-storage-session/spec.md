# agent-storage-session Specification

## Purpose

Define the storage, entry, state persistence, session boundary, legacy-data boundary, and compact-plugin persistence model for the experimental `@yesimbot/agent-runtime` package.
...
#### Scenario: Entry id is distinct from message id

- **WHEN** an entry contains an `AgentMessage`
- **THEN** the storage model MUST preserve both `AgentEntry.id` and `AgentMessage.id`
- **AND** `AgentEntry.id` MUST identify the persisted record while `AgentMessage.id` identifies the semantic message
...
- **AND** the runtime storage contract MUST preserve that entry without requiring core code to understand its semantics
...
Complete assistant messages and tool results MUST be persisted as entries when each step completes. Persisted tool results MUST preserve the model-compatible output shape produced for the model request boundary.
...
- **THEN** storage MUST receive a message or tool-result entry that preserves the normalized tool result content
- **AND** the runtime MUST NOT write the turn id into the message or into `AgentEntry.parentId`

#### Scenario: Cumulative step snapshot deduplication

- **WHEN** step output persistence receives a response snapshot that includes messages already persisted for the current model call
- **THEN** storage MUST NOT receive duplicate entries for those already-persisted assistant messages or tool results
...
- **AND** the event data MUST be an `AgentInternalEvent` carrying the relevant `turnId`
