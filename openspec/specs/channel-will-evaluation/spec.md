...
ChannelRuntime MUST evaluate Will after persisting and publishing the committed input. A decision MUST be either `wait` or `trigger`; `wait` MUST not start or join an Agent turn.
...
- **WHEN** Will returns `trigger` while the Agent is idle
- **THEN** ChannelRuntime MUST start one turn and expose its output

#### Scenario: Will triggers a busy runtime
- **WHEN** Will returns `trigger` while the Agent has an active turn
...
Core MUST publish `yesimbot/will` after each completed decision. When the first output of a turn is successfully delivered, Core MUST notify that runtime's Will engine once. It MUST not notify Will for failed, aborted, skipped, or undelivered turns.

#### Scenario: First output is delivered
- **WHEN** Gateway successfully delivers the first segment of a turn
