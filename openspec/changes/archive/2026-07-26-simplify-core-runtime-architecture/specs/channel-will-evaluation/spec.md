# channel-will-evaluation Delta Specification
...
**Reason**:
Core no longer exposes a custom Will factory. Frozen runtime configuration provides the supported routing and willingness choices through the internal WillEngine construction path.

**Migration**:
Remove calls to `ctx.yesimbot.registerWill()` and implementations of `Will.Factory`. Select `config.will.engine` as `routing` or `willingness` before creating or explicitly reloading a channel runtime.

### Requirement: Will Factory Replacement

**Reason**:
Factory registration and replacement no longer exist, so factory disposal cannot invalidate runtimes.

**Migration**:
Remove factory disposers and replacement handling. Use explicit `ctx.yesimbot.reload(scope)` after changing supported runtime configuration.
