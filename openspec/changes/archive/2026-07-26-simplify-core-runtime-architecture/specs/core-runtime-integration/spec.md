# core-runtime-integration Delta Specification
...
When WillEngine triggers an idle Agent, ChannelRuntime MUST own the sole consumer of the Agent internal stream and MUST expose only complete renderable assistant messages as `AsyncIterable<ChannelRuntime.Output>`. It MUST NOT expose token deltas, tool events, or raw Agent internal events to Gateway.
...
Resolver, persistence, WillEngine, Agent, observation listener, projection, delivery, reload-required, and reload-in-progress failures MUST retain their distinct diagnostics and MUST NOT be mislabeled as one another. A failure in one channel MUST NOT stop another channel runtime.

#### Scenario: WillEngine observer throws
- **WHEN** a `yesimbot/will` listener throws
- **THEN** core MUST record a listener diagnostic
- **AND** it MUST preserve and execute the WillEngine decision
...
**Reason**:
An ordinary event no longer initiates a lifecycle transition. A cached shared-runtime `selfId` mismatch fails closed and requires explicit reload.

**Migration**:
After changing a shared channel's Koishi assignee, call `ctx.yesimbot.reload(scope)` before routing events for the new assignee. Do not rely on event-triggered Runtime replacement.

### Requirement: Handover Backpressure

**Reason**:
Core no longer runs automatic handovers, waiter queues, route retries, or generation-based replacement.

**Migration**:
Remove handling for the five-event handover limit and automatic retry behavior. Treat reload-required and reload-in-progress errors as explicit operator-action diagnostics; reload routes fail without retry.
