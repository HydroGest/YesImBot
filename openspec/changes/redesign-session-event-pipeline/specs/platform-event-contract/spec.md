...
**Reason**: Accepted non-message events now share channel persistence and Will evaluation with messages.

**Migration**: Return EventRecord from SessionResolver so ChannelRuntime can create Event.

### Requirement: Extensible Typed Event Variants

**Reason**: `PlatformEventVariants` is replaced by Satori-shaped `EventMap` shared by messages and non-message events.

**Migration**: Augment `EventMap` and provide the required Satori resources for the event discriminant.

### Requirement: Event-Only Public Publication

**Reason**: The platform publication API is removed. Accepted events enter the authoritative runtime path before observation.

**Migration**: Register a SessionResolver for inbound events and observe committed Event values with `ctx.on("yesimbot/event", ...)`.

### Requirement: Event Content Has No Generic Renderer

**Reason**: Frozen model content is stored on EventRecord and projected from the common Event custom message.

**Migration**: Produce optional frozen content during Session resolution and let core project it locally.
