...
- **THEN** the Agent MUST create it only after it has canonicalized the request to either a once instant or a cron expression
- **AND** the Agent MUST request clarification instead of persisting an ambiguous time or recurrence
...
- **THEN** the plugin MUST prevent future submission for that Schedule
- **AND** it MUST retain the rule and latest result
...
At a due occurrence, the plugin MUST construct an EventRecord with `eventType: "schedule.due"` and submit it only through `ctx.yesimbot.trigger()`. The record MUST contain the stored platform, self id, channel id, actual submission time, untrusted schedule text, and structured schedule id, title, rule kind, and scheduled occurrence fields. The plugin MUST NOT construct or retain a Session, synthesize a MessageRecord, call RuntimeManager directly, consume an Agent output stream, or send through Bot directly.
...
- **THEN** the plugin MUST submit one `schedule.due` EventRecord through the Core trigger facade
- **AND** the existing Core trigger path MUST append and observe the EventRecord before it starts the Agent turn
...
- **THEN** the plugin MUST submit the EventRecord through the same Core trigger facade
...
Before calling Core trigger, the plugin MUST durably record the due occurrence as `lastResult.status: "submitting"` and advance the Schedule beyond that occurrence. The plugin MUST submit an occurrence at most once within one Koishi process. A normal trigger resolution MUST produce `accepted`; a trigger rejection MUST produce `failed`.

#### Scenario: Duplicate timer wake does not duplicate a trigger

- **WHEN** the scheduler processes the same due occurrence more than once
- **THEN** only the first successful durable `submitting` transition MUST call `ctx.yesimbot.trigger()`
- **AND** every later attempt for that occurrence MUST produce no additional trigger call
...
#### Scenario: Restart after a missed one-time schedule

- **WHEN** the plugin starts after a once Schedule's execution instant has passed without a durable submission
- **THEN** the plugin MUST mark the Schedule completed with a missed result
- **AND** it MUST NOT call Core trigger for that occurrence

#### Scenario: Restart after a missed cron schedule

- **WHEN** the plugin starts after one or more cron occurrences have passed
- **THEN** the plugin MUST record one missed result
- **AND** it MUST arm only the first occurrence strictly later than the current time
- **AND** it MUST NOT replay any elapsed occurrence

### Requirement: Bounded Schedule Resource Use

The plugin MUST enforce a maximum of 20 enabled Schedules per channel, a minimum cron interval of 15 minutes, a maximum title length of 120 characters, a maximum prompt length of 2000 characters, and no more than five concurrent Core trigger calls. The plugin MUST NOT create a delayed submission backlog.

#### Scenario: Schedule count or rule limit is exceeded

- **WHEN** an operation would exceed a channel count, text-size, or cron-frequency limit
- **THEN** the plugin MUST reject the operation before changing Schedule persistence or timer state

#### Scenario: No trigger slot is available at a due occurrence

- **WHEN** a due occurrence has no available concurrent trigger slot
- **THEN** the plugin MUST record that occurrence as missed
- **AND** it MUST advance or complete the Schedule according to its rule
- **AND** it MUST NOT queue the occurrence for later submission
...
#### Scenario: Delivery fails after a trigger is accepted
