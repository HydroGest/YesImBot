...
function createSingleToolCallModel() {
  ...
  let callCount = 0;
  const observedPrompts: Array<LanguageModelV3CallOptions["prompt"]> = [];
  const observedToolNames: string[][] = [];
  ...
  return {
    ...
    observedPrompts,
    observedToolNames,
  } as unknown as LanguageModelV3 & { observedPrompts: Array<LanguageModelV3CallOptions["prompt"]>; observedToolNames: string[][] };
}
...
function createFinalizeToolLoopModel(input = "{}") {
  ...
  return {
    ...
    observedPrompts,
  } as unknown as LanguageModelV3 & { observedPrompts: Array<LanguageModelV3CallOptions["prompt"]> };
}
