...
declare module "../src/index.js" {
  interface AgentCustomMessages {
    ...
    "example.user": { id: string; timestamp: number; role: "user"; content: string; source: "example" };
  }

  interface AgentCustomEntries {
    "example.entry": { count: number };
  }
  ...
  interface AgentCustomChannelEvents {
    example: { type: "example.updated"; data: { ok: true } };
  }
}

describe("public types", () => {
  it("accepts declaration-merged custom surfaces", () => {
    const message: AgentCustomMessage<"example.custom"> = { id: "msg_custom", timestamp: 1, role: "custom", type: "example.custom", data: { text: "ok" } };
    const agentMessage: AgentMessage = message;
    const entry: AgentEntry<"example.entry"> = { id: "entry_custom", type: "example.entry", data: { count: 1 }, timestamp: 1 };
    const internalEventInit: AgentInternalEventInit = { type: "agent.init" };
    const internalEvent: AgentInternalEvent = { id: "evt_init", type: "agent.init", timestamp: 1 };
    const internalChannelEvent: AgentCustomChannelEvent<"internal"> = { id: "evt_init", type: "agent.init", timestamp: 1 };
    const customEvent: AgentCustomChannelEvent<"example"> = { type: "example.updated", data: { ok: true } };
    const state: AgentState = { version: 1, exampleFlag: true };
    const plugin: AgentPlugin = { name: "example" };
  ...
  it("keeps merge surfaces importable and typed", () => {
    ...
    expectTypeOf<AgentInternalEvent>().toMatchTypeOf<{ id: string; timestamp: number; type: string }>();
    ...
    expectTypeOf<AgentCustomMessage<"example.user">>().toMatchTypeOf<{ id: string; timestamp: number; role: "user"; content: string; source: "example" }>();
  ...
  it("infers and constrains registered custom message data", () => {
    ...
    expectTypeOf(valid).toMatchTypeOf<{ id: string; timestamp: number; role: "custom"; type: "example.custom"; data: { text: string } }>();
