...
describe("loadSkills", () => {
  ...
  beforeEach(async () => {
    basePath = await mkdtemp(path.join(tmpdir(), "yesimbot-workspace-skills-"));
  });
  ...
  it("discovers SKILL.md roots and nested skills", async () => {
    const csv = path.join(basePath, "csv");
    const nested = path.join(basePath, "nested");
    await mkdir(csv, { recursive: true });
    await mkdir(nested, { recursive: true });
    await writeFile(path.join(csv, "SKILL.md"), "---\nname: csv\ndescription: CSV processing\n---\n# CSV");
  ...
  it("warns for missing paths and keeps other skills", async () => {
    const good = path.join(basePath, "good");
    await mkdir(good, { recursive: true });
    await writeFile(path.join(good, "SKILL.md"), "---\nname: good\ndescription: Good\n---\n# Good");

    const result = await loadSkills({ skillPaths: [path.join(basePath, "missing"), good], cwd: basePath });
