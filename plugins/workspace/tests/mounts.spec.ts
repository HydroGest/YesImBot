...
describe("workspace mount validation", () => {
  ...
  it("resolves relative sources against the base directory and creates rw directories", async () => {
    const baseDir = await mkdtemp(path.join(tmpdir(), "yesimbot-mounts-base-"));
    temporaryRoots.push(baseDir);

    const [mount] = await normalizeMounts([{ source: "created/data", target: "/data/", mode: "rw" }], baseDir);

    expect(mount).toEqual({ source: await realpath(path.join(baseDir, "created", "data")), target: "/data", mode: "rw" });
    await expect(stat(path.join(baseDir, "created", "data"))).resolves.toMatchObject({ isDirectory: expect.any(Function) });
  });

  it("requires existing directories for ro and overlay sources and canonicalizes them", async () => {
    const baseDir = await mkdtemp(path.join(tmpdir(), "yesimbot-mounts-base-"));
    temporaryRoots.push(baseDir);
    await mkdir(path.join(baseDir, "docs"));
    ...
    expect(mounts).toEqual([
      { source: await realpath(path.join(baseDir, "docs")), target: "/docs", mode: "ro" },
      { source: await realpath(path.join(baseDir, "docs")), target: "/overlay", mode: "overlay" },
    ]);
  ...
  it.each([
    ...
    const baseDir = await mkdtemp(path.join(tmpdir(), "yesimbot-mounts-conflict-"));
    temporaryRoots.push(baseDir);
