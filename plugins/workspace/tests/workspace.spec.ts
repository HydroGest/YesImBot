...
async function tmpRoot(name: string): Promise<string> {
  return mkdtemp(path.join(tmpdir(), `yesimbot-${name}-`));
}
...
describe("Workspace filesystem", () => {
  ...
  it("persists writes in the default channel workspace", async () => {
    ...
    await expect(readFile(path.join(root, "report.txt"), "utf8")).resolves.toBe("hello");
  });
  ...
  it("mounts overlay paths as copy-on-write", async () => {
    ...
    await expect(readFile(path.join(repo, "package.json"), "utf8")).resolves.toContain("demo");
  });
