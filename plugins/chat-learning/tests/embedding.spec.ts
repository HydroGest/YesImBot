...
describe("embedding", () => {
  it("builds a pattern embedding map in response and initiation order", async () => {
    ...
    const map = await buildPatternEmbeddingMap(
      { logger: () => ({ warn: vi.fn<() => void>() }), yesimbot: { model: { resolveEmbedding: () => ({}) } } } as never,
      config,
      [response],
      [],
    );
    ...
    expect(mocks.embedMany).toHaveBeenCalledWith(expect.objectContaining({ model: {}, values: ["确实"], maxRetries: 0 }));
  });
