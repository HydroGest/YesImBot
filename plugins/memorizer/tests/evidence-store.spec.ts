...
describe("EvidenceStore", () => {
  it("persists, deduplicates, merges, and removes message evidence", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "yesimbot-evidence-"));
    roots.push(root);
  ...
  it("does not overwrite corrupt evidence JSON", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "yesimbot-evidence-corrupt-"));
    roots.push(root);
    const filePath = path.join(root, "evidence", "broken.json");
