...
async function makeStore(dir: string, now = 1_000): Promise<GlobalBrainStore> {
  const store = createGlobalBrainStore({
    filePath: path.join(dir, "brain.jsonl"),
    maxDigestThreads: 5,
    maxDigestReplies: 5,
    now: () => now,
...
async function withTempDir<T>(fn: (dir: string) => Promise<T>): Promise<T> {
  const dir = await mkdtemp(path.join(tmpdir(), "global-brain-store-"));
  try {
    return await fn(dir);
  } finally {
...
describe("GlobalBrainStore", () => {
  ...
  it("reloads persisted threads and replies", async () => {
    await withTempDir(async (dir) => {
      ...
      const reopened = createGlobalBrainStore({ filePath: path.join(dir, "brain.jsonl"), maxDigestThreads: 5, maxDigestReplies: 5 });
      await reopened.init();
  ...
  it("skips invalid persisted records with a warning", async () => {
    await withTempDir(async (dir) => {
      const filePath = path.join(dir, "brain.jsonl");
