...
export function formatBrainDigest(digest: BrainDigest, maxContentLength: number): string | undefined {
  ...
  for (const thread of digest.threads) {
    ...
    lines.push(
      `[全局脑] ${KIND_LABELS[thread.kind]}：${truncate(thread.content, maxContentLength)}（标签：${tags}，id=${thread.id}；详情用 brain_read 查看）`,
    );
  }

  for (const item of digest.replies) {
    lines.push(
      `[全局脑] 你发布的 ${KIND_LABELS[item.thread.kind]} 有 ${item.replies.length} 条新回复（id=${item.thread.id}；详情用 brain_read 查看）`,
    );
  }
