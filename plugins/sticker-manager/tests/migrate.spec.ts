...
describe("migrateV3", () => {
  ...
  it("imports matching v3 records into the active global scope", async () => {
    baseDir = await mkdtemp(path.join(tmpdir(), "sticker-migrate-"));
    const oldDir = path.join(baseDir, "old");
    const dataDir = path.join(baseDir, "data");
    const oldFile = path.join(oldDir, "sticker.png");
  ...
  it("dry-run reports imports without writing rows", async () => {
    baseDir = await mkdtemp(path.join(tmpdir(), "sticker-migrate-dry-"));
    const oldDir = path.join(baseDir, "old");
    const oldFile = path.join(oldDir, "sticker.png");
    const bytes = new Uint8Array([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, 1, 2, 3]);
    await mkdir(oldDir, { recursive: true });
  ...
  it("registers the v3 model and accepts JSON-string sources", async () => {
    baseDir = await mkdtemp(path.join(tmpdir(), "sticker-migrate-source-"));
    const oldDir = path.join(baseDir, "old");
    const oldFile = path.join(oldDir, "sticker.png");
  ...
  it("migrates bmp and svg files that v3 could store", async () => {
    baseDir = await mkdtemp(path.join(tmpdir(), "sticker-migrate-bmp-svg-"));
    const oldDir = path.join(baseDir, "old");
    const bmpFile = path.join(oldDir, "sticker.bmp");
    const svgFile = path.join(oldDir, "sticker.svg");
    await mkdir(oldDir, { recursive: true });
