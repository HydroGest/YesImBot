...
async function createCardFile(): Promise<string> {
  const root = await mkdtemp(path.join(tmpdir(), "yesimbot-roleplay-plugin-"));
  roots.push(root);
  const cardPath = path.join(root, "card.png");
  await writeFile(
    cardPath,
    createPng({
      spec: "chara_card_v3",
...
describe("RoleplayPlugin", () => {
  it("chooses one random greeting for the card snapshot and renders scope-specific users", async () => {
    const cardPath = await createCardFile();
    const plugins: RoleplayPlugin[] = [];
    const dispose = vi.fn();
    const ctx = {
      baseDir: path.join(cardPath, ".."),
