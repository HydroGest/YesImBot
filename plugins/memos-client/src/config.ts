...
export const memosConfigSchema: Schema<MemosClientConfig> = Schema.object({
  ...
  memoryScope: Schema.union([
    Schema.const("auto").description(
      "Group chats use channel memory, private chats use user memory",
    ),
  ...
  searchPreferenceLimit: Schema.number()
    .default(6)
    .description("Maximum preference memories returned"),
  ...
  searchFilterMode: Schema.union([
    ...
    Schema.const("strict").description(
      "Filter by context, agent, configured tags, and import source",
    ),
  ])
    .default("context")
