...
async function createInputDirectory(): Promise<string> {
  ...
  await writeJson(
    path.join(dir, `Example_Group(${GROUP_ID}).json`),
    createExportFixture({
      name: "Example Group",
      type: "group",
      messages: [
  ...
  await writeJson(
    path.join(dir, "Example_Private.json"),
    createExportFixture({
  ...
  await writeJson(path.join(dir, "nested", "Nested_Should_Not_Load.json"), createExportFixture({ name: "Nested", type: "group", messages: [] }));
  return dir;
}
