...
describe("JsonlUsageHistory", () => {
  it("scans assistant usage and deduplicates entries across session files", async () => {
    const directory = await mkdtemp(path.join(tmpdir(), "yesimbot-usage-jsonl-"));
    ...
    await writeFile(path.join(directory, "a.jsonl"), `${message("a")}\n${message("b")}\n`);
    await writeFile(path.join(directory, "b.jsonl"), `${message("a")}\n`);
