...
function node(message: unknown[], overrides: Record<string, unknown> = {}) {
  return { sender: { user_id: 10001, nickname: "Alice", card: "" }, time: 1_753_888_000, message, raw_message: "[CQ:ignored]", ...overrides };
}
...
describe("createForwardReader", () => {
  it("projects array text exactly instead of reading raw CQ text", async () => {
    ...
    await expect(reader({ forwardId: "forward" })).resolves.toEqual({ messages: [["Alice (10001)", expect.any(String), ["https://example.test/docs"]]] });
  });
  ...
  it("uses an animated emoji placeholder when image subtype is one", async () => {
    ...
    await expect(reader({ forwardId: "forward" })).resolves.toEqual({ messages: [["Alice (10001)", expect.any(String), ["[动画表情]"]]] });
  });
  ...
  it("attaches image summary to animated emoji placeholders when enabled", async () => {
    ...
    await expect(reader({ forwardId: "forward" })).resolves.toEqual({ messages: [["Alice (10001)", expect.any(String), ["[动画表情: 大笑]"]]] });
  });
    ...
    const { reader } = createReader(
      [
        node([
          ...
          { type: "image", data: { summary: "cover", file: "cover.jpg", file_size: "52762", url: "https://private.test/cover.jpg", sub_type: 1 } },
          { type: "text", data: { text: "after" } },
        ]),
        node([{ type: "image", data: { summary: "", file: "one.bin", file_size: "1000" } }]),
        node([{ type: "image", data: { summary: "", file: "bad.bin", file_size: "-1" } }]),
        node([{ type: "image", data: { summary: "", file: "unsafe.bin", file_size: "9007199254740992" } }]),
      ],
  ...
  it("renders persisted forward images as local asset URIs", async () => {
    const getForwardMsg = vi.fn(async () => [
      node([{ type: "image", data: { summary: "cover", file: "cover.jpg", file_size: "1000", src: "https://private.test/cover.jpg" } }]),
    ]);
    ...
    const result = await reader({ forwardId: "forward" });
    expect(result).toEqual({ messages: [["Alice (10001)", expect.any(String), ["[图片：asset://asset-0]"]]] });
  ...
  it("renders unpersisted forward images as text placeholders", async () => {
    const getForwardMsg = vi.fn(async () => [
      node([
        { type: "image", data: { summary: "cover", file: "cover.jpg", file_size: "1000", url: "https://private.test/cover.jpg" } },
        { type: "image", data: { summary: "second", file: "second.jpg", file_size: "2000", url: "https://private.test/second.jpg" } },
      ]),
    ]);
    ...
    const result = await reader({ forwardId: "forward" });
    expect(result).toEqual({ messages: [["Alice (10001)", expect.any(String), ["[图片：asset://asset-0][图片]"]]] });
  });

  it("caches expanded nested forwards under their forward ID", async () => {
    const { reader, getForwardMsg } = createReader([
      node([{ type: "forward", data: { id: "child-forward", content: [node([{ type: "text", data: { text: "cached child" } }])] } }]),
    ]);

    await expect(reader({ forwardId: "forward" })).resolves.toEqual({ messages: [["Alice (10001)", expect.any(String), [{ forward: "child-forward" }]]] });
    await expect(reader({ forwardId: "child-forward" })).resolves.toEqual({ messages: [["Alice (10001)", expect.any(String), ["cached child"]]] });
  ...
  it("returns a clear error result when neither cache nor OneBot has a forward", async () => {
    ...
    await expect(reader({ forwardId: "missing" })).resolves.toEqual({ error: "未找到合并转发消息: missing" });
  });
  ...
  it("caches normalized records while paging from the requested offset", async () => {
    ...
    await expect(reader({ forwardId: "forward", offset: 1 })).resolves.toEqual({ messages: [["Alice (10001)", expect.any(String), ["b".repeat(3500)]]] });
  ...
  it("retries failed loads and keeps cache entries independent by forward ID", async () => {
    ...
    const reader = createForwardReader({ getForwardMsg } as OneBotInternal, { parseImages: false, maxForwardPageChars: 6000, attachImageSummary: true });

    await expect(reader({ forwardId: "first" })).rejects.toThrow("temporary");
    await expect(reader({ forwardId: "first" })).resolves.toEqual({ messages: [["Alice (10001)", expect.any(String), ["first"]]] });
    await expect(reader({ forwardId: "second" })).resolves.toEqual({ messages: [["Alice (10001)", expect.any(String), ["second"]]] });
